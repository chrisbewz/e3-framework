...
{
    public static async Task<IEnumerable<IAttributeBase>> ConfigureAttributes(this IComponentBase<IDeviceInterface> component)
    {
        var id = component.Id;
        ...
        var parser = new Task<IEnumerable<IAttributeBase>>(() =>
        {
            ...
            (attIds as object[])?.AsEnumerable()?.AsParallel().ForAll(attId =>
            {
                attributesRead.Add(Attribute.FromId((int)attId));
            });
